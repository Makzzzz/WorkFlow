...
class SolutionService:
    ...
    async def submit_solution(self, task_id: int, solution_data: SolutionCreate, user_id: int) -> Solution:
        sol = await self.solution_repo.create_solution(solution_data, user_id, task_id)
        return sol
    ...
    async def update_solution(self, solution_id: int, solution_data: SolutionUpdate, user_id: int) -> Solution:
        updated = await self.solution_repo.update_solution(solution_id, solution_data)
        if not updated:
            raise HTTPException(status_code=404, detail="Solution not found")
        return updated
